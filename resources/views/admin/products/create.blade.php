...
<div class="max-w-4xl mx-auto mt-8 bg-[#FFFFFF] rounded-3xl shadow-xl overflow-hidden">
    ...
    <div class="bg-[#2ECCB0] p-6 rounded-t-3xl">
        ...
        <p class="text-[#2E2E2E]/80 mt-1">
            {{ isset($product) ? 'Update your product details below.' : 'Fill in the product details to add a new product.' }}
        </p>
    </div>

    <!-- Form -->
    <form action="{{ isset($product) ? route('admin.products.update', $product) : route('admin.products.store') }}"
          method="POST" enctype="multipart/form-data" class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col">
                ...
                @if(isset($product) && $product->image)
                    <img src="{{ asset('storage/'.$product->image) }}" class="w-40 h-40 mt-3 object-cover rounded-xl border border-[#2ECCB0] shadow-lg">
                @endif
